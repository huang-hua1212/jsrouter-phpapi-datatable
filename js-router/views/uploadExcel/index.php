...
<body>
<div class="container">
    <input type="file" id="file-uploader" data-target="file-uploader" accept=".csv,.xls"  multiple="multiple"/>
  ...
  $('#file-uploader').on('change', function(e) {
    if(e.target.files.length > 1) {
      ...
      fileReader.addEventListener("load", function (e) {          //等fileReader.readAsDataURL讀完全部檔案後，才會進行fileReader.addEventListener之迴圈(巡迴)，並且this.result會依序更換(this.result為每個檔案)
          ...
          $.ajax({
            url: 'http://localhost:8888/MAMP/projects/jsrouter-phpapi-datatable/router.php/upload-single-excel',
            async: true,
            method: 'POST', 
            data: formData,
            cache: false,
